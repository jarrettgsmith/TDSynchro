...
foreach ($computer in $computers) {
    ...
    Invoke-Command -Session $session -ScriptBlock {
        ...
        $file = "$using:sourceDir\TDSynchro\SimpleClient.toe"
        Write-Host "Running TouchDesigner from $using:sourceDir"
        Set-Location $using:location
        & ../../PSTools/psexec -accepteula -s \\localhost -i $using:sessionId -d -u "NT AUTHORITY\NETWORK SERVICE" $app $file *> $null
    }
